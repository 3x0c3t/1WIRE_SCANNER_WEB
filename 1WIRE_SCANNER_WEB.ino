*/
=============================================================
Fichier     : 1WIRE_SCANNER_WEB
Auteur      : 3x0c3t
Version     : 1.1
Date        : 20250715-23:00
Description : Scanner 1WIRE + WebServer + WebPage
=============================================================
*/
...
const byte ONEWIRE_BUS_PIN = 13;
OneWire ds(ONEWIRE_BUS_PIN);
ESP8266WebServer server(80);

String htmlContent = "<html><head><meta charset='utf-8'><title>Résultats 1-Wire</title></head><body><h1>Résultats 1-Wire</h1><pre>";
...
void setup() {
  ...
  server.on("/", []() {
    server.send(200, "text/html", htmlContent + "</pre></body></html>");
  });
...
void loop() {
  ...
  if (OneWire::crc8(address, 7) != address[7]) {
    Serial.print(F(" (CRC invalide)"));
    line += "(CRC invalide)";
  }

  String desc = get_onewire_family_description(address);
  Serial.println(desc);
  line += " " + desc + "\n";

  htmlContent += line;
  server.handleClient();
  delay(1000); // pour lisibilité et éviter spam
}
